...
community_sim_limsim <- function(tree,
                          reps=100,
                          comm_size = 30,
                          paramFUN = `rnorm`,
  ...
  for(i in 1:reps){
    ...
    whichsp <- seq(n.sp)
    closest <- NULL
    for(m in 1:(n.sp - comm_size)){
      if(!is.null(closest)){whichsp <- whichsp[-which(mytree$tip.label %in% closest)]}
      closest <- c(closest, names(which.min(colMeans(as.matrix(dist(bm[whichsp,], upper=T))))))
      whichsp <- seq(n.sp)
    }

    mycomm <- rep(1, n.sp)
    mycomm[which(mytree$tip.label %in% closest)] <- 0
...
mytree <- drop.extinct(sim.bdtree(.8,0.2,"taxa",128))
commsize <- 32
#Broken into 200 25-sim chunks for computational reasons
for(i in 1:80){
  newsim <- community_sim_limsim(mytree,
                  reps=25,
                  comm_size = commsize,
                  param_list = list(n=1,mean=0,sd=0.5),
                  #param_bounds = c(-.2,.2),
